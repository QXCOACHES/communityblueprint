...
export default function CommunityBlueprint() {
  ...
  const handleGenerateStatement = async () => {
    ...
    try {
      ...
      setStatement(result);
      setCard(5); // go to statement reveal card
    } catch (e) {
  ...
  // Section 1 cards
  const s1Cards = [
    "hook", "category", "person", "pain", "result", "statement"
  ];
  const s2Cards = [
    "pointA", "phases", "vision"
  ];
  const s3Cards = ["scalable", "cta"];
  ...
  const wrap = (children, showProgress = true) => (
    <div style={{ background: "#0a0a0a", minHeight: "100vh", fontFamily: "'DM Sans',sans-serif", color: "#e8e4dc" }}>
      ...
      <div style={{ maxWidth: 560, margin: "0 auto", padding: "28px 20px 60px" }}>
        {showProgress && <ProgressBar current={progressCard} total={totalCards} />}
    ...
    <div style={{ background: "#0a0a0a", minHeight: "100vh", fontFamily: "'DM Sans',sans-serif", color: "#e8e4dc", display: "flex", alignItems: "center" }}>
      ...
      <div style={{ maxWidth: 560, margin: "0 auto", padding: "40px 20px" }} className="fu">
        ...
        <div style={{ fontSize: 13, color: "#666", lineHeight: 1.9, marginBottom: 32, borderLeft: `2px solid ${ORANGE}`, paddingLeft: 14 }}>
          ...
          <span style={{ color: "#888" }}>We'll generate your positioning statement using AI, then sketch your community phase map.</span>
        </div>
      ...
      switch (currentCardName) {
          ...
          <>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
              <div>
                ...
                <TextArea value={a.person || ""} onChange={v => set("person", v)} placeholder="e.g. burnt-out therapists pivoting to coaching, high-performing moms with autoimmune issues, male entrepreneurs post-divorce..." rows={2} />
              </div>
              <div>
        ...
        case "pain": return (
          <>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
              <div>
                <div style={{ fontSize: 11, color: "#555", marginBottom: 6 }}>What's the visceral pain they're desperate to escape? Be specific — what does it feel like day-to-day?</div>
                <TextArea value={a.pain || ""} onChange={v => set("pain", v)} placeholder="e.g. waking up dreading their inbox, feeling like a fraud despite 10 years of experience, watching less qualified people charge more..." rows={3} />
              </div>
              <div>
                <div style={{ fontSize: 11, color: "#555", marginBottom: 6 }}>What's the story or belief keeping them stuck?</div>
                <TextArea value={a.belief || ""} onChange={v => set("belief", v)} placeholder="e.g. 'I need more certifications first', 'I'm not ready to charge that', 'nobody will pay that much for what I do'..." rows={2} />
              </div>
            </div>
          </>
        );
          ...
          <>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
              <div>
                ...
                <TextArea value={a.result || ""} onChange={v => set("result", v)} placeholder="e.g. replace their 9-5 income, lose 20lbs of inflammation, go from 0 to first $10k month, book 5 new clients in 60 days..." rows={2} />
              </div>
              <div>
        ...
        case "statement": return (
          <>
            ...
            {statement ? (
              <>
                <div style={{
                  background: "#111", border: `1px solid ${ORANGE}`,
          ...
          <>
            ...
            <div style={{ background: "#111", border: "1px solid #1e1e1e", borderRadius: 8, padding: "14px", marginTop: 16 }}>
              <div style={{ fontSize: 11, color: "#555" }}>
                Point B is the result you defined in your positioning statement:<br />
                <span style={{ color: "#888", fontStyle: "italic" }}>"{statement || "your result"}"</span>
              </div>
            </div>
          </>
        );
        ...
        case "phases": return (
          <>
            <div style={{ fontSize: 10, color: ORANGE, letterSpacing: "0.15em", textTransform: "uppercase", marginBottom: 20 }}>Section 2 — Your Phase Map</div>
            <Label sub="What are the major milestones between Point A and Point B? Each phase is a chunk of transformation — name it and describe what happens in it. Minimum 3, maximum 7.">
            ...
            <div style={{ fontSize: 11, color: "#555", lineHeight: 1.7, marginBottom: 16, borderLeft: `2px solid ${ORANGE}`, paddingLeft: 10 }}>
              Think in phases, not steps. Each phase should answer: <span style={{ color: "#888" }}>"After they complete this, what's the next problem they'll face?"</span>
            </div>
        ...
        case "vision": return (
          <>
            ...
            <div style={{ fontSize: 12, color: "#555", lineHeight: 1.8 }}>
              Each of these phases becomes its own course inside your Skool community — supported by templates, resources, and a custom GPT trained on your methodology. Keep reading to see how.
            </div>
          </>
        );

        // ── SECTION 3: THE SKOOL MODEL ──────────────────────────────────────
        ...
        case "scalable": return (
          <>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 10, marginBottom: 20 }}>
              {[
                { icon: "🎓", label: "A course", sub: "Video lessons that walk your members through each phase — so you're not re-teaching the same thing on every 1-on-1 call." },
                ...
                { icon: "🤖", label: "A custom GPT", sub: "Trained on your methodology. Answers questions, coaches through obstacles, and delivers your thinking at scale — 24/7." },
                { icon: "📞", label: "Group coaching calls", sub: "Instead of 10 separate 1-on-1 calls, one group session serves everyone in the same phase simultaneously. Your time stops being the bottleneck." },
              ].map((item, i) => (
            ...
            <div style={{ background: "#0f0f0f", border: `1px solid ${OB}`, borderRadius: 10, padding: "16px" }}>
              <div style={{ fontSize: 12, color: "#777", lineHeight: 1.9 }}>
                The piece we haven't covered yet is how you <span style={{ color: "#e8e4dc" }}>fill</span> the community, price it, and build the funnel that keeps it growing month over month.<br /><br />
                <span style={{ color: "#555", fontSize: 11 }}>That's what the call is for.</span>
              </div>
            </div>
          </>
        );
    ...
    return wrap(
      <>
        ...
        <div style={{ marginTop: 4 }}>
          {currentCardName !== "result" || !generating ? (
            <NextBtn
              label={
                currentCardName === "result" ? "Generate My Statement →" :
                currentCardName === "cta" ? null :
                card === totalCards - 1 ? "Finish →" : "Next →"
              }
              onClick={currentCardName === "cta" ? null : next}
              disabled={!canNext() || currentCardName === "cta"}
              loading={generating}
            />
          ) : (
            <div style={{ textAlign: "center", padding: "16px 0" }}>
              <div className="pulse" style={{ fontSize: 11, color: ORANGE, letterSpacing: "0.12em" }}>Writing your positioning statement...</div>
            </div>
          )}
        </div>
        <div style={{ marginTop: 10 }}>
          {card > 0 && currentCardName !== "cta" && <BackBtn onClick={back} />}
        </div>
      </>
    );
  }
